...
const input = path.join(os.homedir(), 'instagram-collector', `${account}-latest12-full.json`);
...
const bestPost = [...posts].filter(p => typeof p.likes === 'number').sort((a, b) => ((b.likes || 0) - (a.likes || 0)) || ((b.comments || 0) - (a.comments || 0)))[0] || null;
const result = {
  account: raw.account,
  ...
  engagement: {
    ...
    total_likes_last12: posts.reduce((s, p) => s + (p.likes || 0), 0),
    total_comments_last12: posts.reduce((s, p) => s + (p.comments || 0), 0)
  },
  content_breakdown: {
