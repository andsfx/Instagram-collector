...
export function RankingGrowthPresentation({ data }: { data: DashboardRecord }) {
                ...
                {SORT_LABELS.map((column) => (
                  ...
                  >
                    <button type="button" className="inline-flex items-center gap-1.5 bg-transparent p-0 text-inherit" onClick={() => handleSort(column.key)}>
                      ...
                      <span className={`hidden text-[0.72rem] ${sortKey === column.key ? 'text-brand' : 'text-text-soft'}`} aria-hidden="true">
