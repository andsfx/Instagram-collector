...
(async () => {
  ...
  if (!account) {
    console.error('Usage: node scripts/collect/collect-instagram-posts-full.js <account>');
    process.exit(1);
  }

  const repoRoot = path.resolve(__dirname, '..', '..');
  const profileDir = path.join(os.homedir(), 'instagram-collector', 'profile-default');
  const outputDir = path.join(repoRoot, 'data', 'raw', 'posts');
  const legacyOutput = path.join(repoRoot, `${account}-latest12-full.json`);
  const output = path.join(outputDir, `${account}-latest12-full.json`);
  ensureDir(outputDir);
  ...
  const writeOutputs = () => {
    ...
    fs.writeFileSync(legacyOutput, JSON.stringify(result, null, 2), 'utf8');
  };
